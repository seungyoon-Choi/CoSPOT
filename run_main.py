...
parser.add_argument('--prompt_type', type=str, default='frequency', help = 'type of text prompt')
parser.add_argument('--frequency_ratio', type=int, default=4, help='')
parser.add_argument('--wavelet_level', type=int, default=2, help='')
...
parser.add_argument('--eval_batch_size', type=int, default=24, help='batch size of model evaluation')
...
parser.add_argument('--is_train', type=int, default=1, help="")

args = parser.parse_args()
ddp_kwargs = DistributedDataParallelKwargs(find_unused_parameters=True)
deepspeed_plugin = DeepSpeedPlugin(hf_ds_config='./ds_config_zero.json')
...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader)):
            ...
            if (i + 1) % 100 == 0:
                accelerator.print(
                    "\titers: {0}, epoch: {1} | loss: {2:.7f} | mae loss: {3:.7f}".format(i + 1, epoch + 1, loss.item(), mae_loss.item()))
        ...
        vali_loss, vali_mae_loss = vali(args, accelerator, model, vali_data, vali_loader, criterion, mae_metric, is_train=0)
        accelerator.print(
            "Epoch: {0} | Train Loss: {1:.7f} Train MSE Loss: {2:.7f} Train MAE Loss: {3:.7f} Vali Loss: {4:.7f} Vali MAE Loss: {5:.7f}".format(
                epoch + 1, train_loss, train_mse_loss, train_mae_loss, vali_loss, vali_mae_loss))
    ...
    test_loss, test_mae_loss = vali(args, accelerator, model, test_data, test_loader, criterion, mae_metric, is_train=args.is_train)
